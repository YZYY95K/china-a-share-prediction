...
def objective(trial):
    params = {
        ...
        'reg_alpha': trial.suggest_float('reg_alpha', 0.1, 1.0),  # 更强正则化
...
# 第二层特征
X_stack_train_l2 = np.hstack([lgb_stack_train, xgb_stack_train, cat_stack_train])
...
# 添加原始特征到第三层
X_stack_train_l3 = np.hstack([X_stack_train_l2, X_train_selected[:, :50]])
X_stack_val_l3 = np.hstack([X_stack_val_l2, X_val_selected[:, :50]])
...
unique_dates_val = sorted(val_data['dateid'].unique())
n_val_dates = len(unique_dates_val)
window_size = n_val_dates // 3

rolling_scores = []
for i in range(3):
    start_idx = i * window_size
    end_idx = (i + 1) * window_size if i < 2 else n_val_dates
    
    val_dates_window = unique_dates_val[start_idx:end_idx]
    val_window = val_data[val_data['dateid'].isin(val_dates_window)]
    
    if len(val_window) > 0:
        X_val_window = val_window[selected_features].fillna(0).values.astype('float32')
        y_val_window = val_window['LabelA'].values
        
        pred = lgb_model.predict(X_val_window)
        r2 = r2_score(y_val_window, pred)
        rolling_scores.append(r2)
        print(f"  窗口{i+1}: R² = {r2:.6f}")

print(f"  平均R²: {np.mean(rolling_scores):.6f}")
print(f"  标准差: {np.std(rolling_scores):.6f}")
...
stack_blend_pred = 0.5 * weighted_pred + 0.3 * stack_l2_pred + 0.2 * stack_l3_pred
...
try:
    ...
    shap_values = explainer.shap_values(X_val_selected[:5000])
    
    shap_importance = pd.DataFrame({
        'feature': selected_features[:len(shap_values[0])],
...
print(f"\n=== V15 核心优化 ===")
print(f"  1. ⭐⭐⭐⭐⭐ 更多训练数据: {N_CHUNKS}个chunk")
print(f"  2. ⭐⭐⭐⭐⭐ 多层Stacking: 3层")
print(f"  3. ⭐⭐⭐⭐⭐ 特征交互: {len(interaction_features)}个")
print(f"  4. ⭐⭐⭐⭐ 滚动窗口验证: ✅")
print(f"  5. ⭐⭐⭐⭐ SHAP分析: ✅")
print(f"  6. ⭐⭐⭐⭐ 更强正则化: ✅")
print(f"  7. 傅里叶特征: {len(fourier_features)}个")
print(f"  8. 跨股票特征: {len(cross_features)}个")
print(f"  9. 特征选择: {len(selected_features)}个")
print(f"  10. Optuna: {OPTUNA_TRIALS}次试验")
