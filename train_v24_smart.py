...
🏆 V24 极限优化版 - 增强搜索 + 智能特征选择

包含优化:
1. ⭐⭐⭐⭐⭐ 20次Bagging
2. ⭐⭐⭐⭐⭐ 智能特征选择 (RFE + Importance)
3. ⭐⭐⭐⭐⭐ 自适应学习率
4. ⭐⭐⭐⭐⭐ 更大搜索空间 (Optuna 200次)
5. ⭐⭐⭐⭐ 更深度特征交互
"""

print("="*70)
print("🏆 V24 极限优化版 - 增强搜索 + 智能特征选择")
print("="*70)
...
lgb_selector = lgb.LGBMRegressor(n_estimators=500, learning_rate=0.05, random_state=42, verbose=-1)
...
print(f"特征选择时间: {time.time() - start_time:.1f}秒")
print("\nTop 15 特征:")
print(importance.head(15))

# ============================================================
# 14. Optuna 200次试验 (更大搜索空间!)
# ============================================================
print("\n[14] Optuna贝叶斯优化 (200次试验)...")
...
# 使用学习率衰减
lgb_adaptive = lgb.LGBMRegressor(**{**lgb_base_params, 'n_estimators': 1500})
lgb_adaptive.fit(
    X_train, y_train_a,
...
lgb_adaptive_pred = lgb_adaptive.predict(X_val_selected)
lgb_adaptive_r2 = r2_score(y_val, lgb_adaptive_pred)
print(f"自适应学习率 R²: {lgb_adaptive_r2:.6f}")
...
print(f"LightGBM(20-Bagging) R²: {lgb_r2:.6f}")
...
level1_pred = np.column_stack([
    ...
    multi_task_pred, lgb_adaptive_pred
])
...
xgb_test_pred = np.mean([m.predict(test_features) for m in xgb_bag_preds], axis=0)
...
del X_train, X_train_full, test_df
gc.collect()
...
all_r2 = {
    ...
    'LightGBM_20Bag': lgb_r2,
    'LightGBM_Adaptive': lgb_adaptive_r2,
    'XGBoost': xgb_r2,
    'CatBoost': cat_r2,
    'MLP': mlp_r2,
    'Ridge': ridge_r2,
...
print(f"\n=== V24 核心优化 ===")
print(f"  1. ⭐⭐⭐⭐⭐ 智能特征选择 (300特征)")
print(f"  2. ⭐⭐⭐⭐⭐ Optuna {OPTUNA_TRIALS}次 (更大空间)")
print(f"  3. ⭐⭐⭐⭐⭐ 自适应学习率")
print(f"  4. ⭐⭐⭐⭐⭐ {N_BAGS}次Bagging")
print(f"  5. ⭐⭐⭐⭐ 更深度特征交互")
