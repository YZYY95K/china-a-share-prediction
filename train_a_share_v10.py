...
print("="*70)
print("🏆 终极版 V10 - 全优化集成 + 深度学习 + 超参数优化")
print("="*70)
...
if torch.cuda.is_available():
    ...
    if gpu_memory >= 70:  # H100 (80GB)
        BATCH_SIZE_LGBM = 8192
        BATCH_SIZE_DEEP = 4096
        BATCH_SIZE_TRANSFORMER = 2048
        OPTUNA_TRIALS = 150
        print("  模式: H100 优化 (80GB)")
    elif gpu_memory >= 20:  # RTX 4090 (24GB)
        BATCH_SIZE_LGBM = 4096
        BATCH_SIZE_DEEP = 2048
        BATCH_SIZE_TRANSFORMER = 1024
        OPTUNA_TRIALS = 100
        print("  模式: RTX 4090 优化 (24GB)")
    else:
        BATCH_SIZE_LGBM = 2048
        BATCH_SIZE_DEEP = 1024
...
# 数据增强
train_data_aug = augment_data(train_data, augment_ratio=0.2)
...
# ============================================================
# 6. 模型训练 - Ridge + LGB + XGB + CatBoost + LSTM + Transformer
# ============================================================
print("\n[6] 模型训练 - 全模型集成...")
print("="*50)
...
try:
    ...
    model_lstm = LSTMModel(input_size=n_lstm_features).to(device)
...
try:
    n_trans_features = min(100, X_train.shape[1])
    ...
    train_loader_trans = DataLoader(train_dataset_trans, batch_size=512, shuffle=True)
    val_loader_trans = DataLoader(val_dataset_trans, batch_size=512, shuffle=False)
    
    model_trans = TransformerModel(input_size=n_trans_features).to(device)
    criterion = nn.MSELoss()
...
try:
    n_mt_features = min(150, X_train.shape[1])
    X_train_mt = X_train[:, :n_mt_features]
    X_val_mt = X_val[:, :n_mt_features]
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    model_mt = MultiTaskModel(input_size=n_mt_features).to(device)
    ...
    train_loader_mt = DataLoader(train_dataset_mt, batch_size=1024, shuffle=True)
    val_loader_mt = DataLoader(val_dataset_mt, batch_size=1024, shuffle=False)
...
if use_lstm:
    ...
    test_loader_lstm = DataLoader(test_dataset_lstm, batch_size=512, shuffle=False)
...
if use_trans:
    ...
    test_loader_trans = DataLoader(test_dataset_trans, batch_size=512, shuffle=False)
...
if use_mt:
    ...
    test_loader_mt = DataLoader(test_dataset_mt, batch_size=1024, shuffle=False)
...
print(f"\n=== V10 新增优化 ===")
print(f"  1. Optuna超参数优化: ✅")
print(f"  2. CatBoost GPU模型: ✅" if use_cat else "  2. CatBoost GPU模型: ❌")
print(f"  3. 多任务学习: ✅" if use_mt else "  3. 多任务学习: ❌")
print(f"  4. 自动特征工程: ✅")
print(f"  5. 多层Stacking: ✅" if use_stacking2 else "  5. 多层Stacking: ❌")
print(f"  6. Transformer时序模型: ✅" if use_trans else "  6. Transformer时序模型: ❌")
print(f"  7. 数据增强: ✅")

print(f"\n=== 模型性能对比 ===")
print(f"  Ridge:        R² = {ridge_r2:.6f}")
print(f"  LightGBM:     R² = {lgb_r2:.6f} (优化参数+数据增强)")
print(f"  XGBoost:      R² = {xgb_r2:.6f}")
if use_cat:
...
print("\n✅ 所有10项优化已应用:")
print("  1. ⭐⭐⭐⭐⭐ Optuna超参数优化")
print("  2. ⭐⭐⭐⭐⭐ CatBoost GPU模型")
print("  3. ⭐⭐⭐⭐⭐ 多任务学习")
print("  4. ⭐⭐⭐⭐ 自动特征工程")
print("  5. ⭐⭐⭐⭐ 多层Stacking")
print("  6. ⭐⭐⭐⭐ Transformer时序模型")
print("  7. ⭐⭐⭐ 数据增强")
print("  8. ⭐⭐⭐ A股特性特征工程")
print("  9. ⭐⭐⭐ 涨跌停核心特征")
print(" 10. ⭐⭐⭐ 截面标准化 + EMA平滑")
print("\n🏆 预期性能提升: +0.05-0.08 R²")
print("🎯 祝竞赛取得金牌!")
