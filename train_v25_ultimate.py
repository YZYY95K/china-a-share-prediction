...
🏆 V25 极限优化版 - 极限搜索 + 多轮伪标签集成

包含优化:
1. ⭐⭐⭐⭐⭐ Optuna 300次 (极限搜索空间)
2. ⭐⭐⭐⭐⭐ 多轮伪标签集成 (5轮)
3. ⭐⭐⭐⭐⭐ 25次Bagging
4. ⭐⭐⭐⭐⭐ 更宽泛的搜索范围
"""
...
USE_GPU = True
TRAIN_RATIO = 0.85
OPTUNA_TRIALS = 300  # 300次极限搜索!
N_BAGS = 25  # 25次!
N_FOLDS = 5
PSEUDO_ROUNDS = 5  # 5轮伪标签!
...
if 'price_change' in train_data.columns and 'market_vol' not in train_data.columns:
...
if use_cat:
    ...
    final_pred = (0.18 * lgb_test_pred + 0.16 * xgb_test_pred + 0.14 * cat_test_pred + 
                 0.10 * mlp_test_pred + 0.08 * ridge_test_pred + 0.14 * multi_task_test +
                 0.20 * pseudo_ensemble_pred)
else:
    final_pred = (0.22 * lgb_test_pred + 0.18 * xgb_test_pred + 
                 0.10 * mlp_test_pred + 0.08 * ridge_test_pred + 0.17 * multi_task_test +
                 0.25 * pseudo_ensemble_pred)
...
print(f"\n=== V25 核心优化 ===")
print(f"  1. ⭐⭐⭐⭐⭐ Optuna {OPTUNA_TRIALS}次 (极限搜索!)")
print(f"  2. ⭐⭐⭐��⭐ 多轮伪标签集成 ({PSEUDO_ROUNDS}轮)")
print(f"  3. ⭐⭐⭐⭐⭐ {N_BAGS}次Bagging")
print(f"  4. ⭐⭐⭐⭐⭐ 更大搜索范围")
